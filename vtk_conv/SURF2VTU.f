...
c       initial data for SPH models                                                                     72
      ...
      real xp(np_max),yp(np_max),zp(np_max),srf(np_max)
      real alp(np_max)
      real dummy
...
c     %LOAD AND READ TIME,DT FROM FILE DT. THE FIRST HEADERLINE IN 
     ...
     &              i_PART_counter_max
          ...
          read(70,*,END = 76) dummy,dummy_int,dummy_int,dummy,
     &                       dummy_int,dummy_int,dummy_int,
     &                       np_all(i_PART_counter)
...
c       % READ IN THE PART FILE FOR EACH FRAME
        ...
        if(ngrab.lt.10) then
           ...
           name_orig='../data_directory/WSURF'//supp1
        end if
        if(ngrab.ge.10.and.ngrab.lt.100) then
           write(supp2,'(i2)') ngrab
           name_orig='../data_directory/WSURF'//supp2
        end if
        if(ngrab.ge.100.and.ngrab.lt.1000) then
           write(supp3,'(i3)') ngrab
           name_orig='../data_directory/WSURF'//supp3
        end if
        if(ngrab.ge.1000.and.ngrab.lt.10000) then
           write(supp,'(i4)') ngrab
           name_orig='../data_directory/WSURF'//supp
        end if
        write(supp,'(i4.4)') ngrab

        name_vtu ='../paraview_files/WSURF'//supp//'.vtu'
...
c       % READ POSITION, VELOCITY, DENSITY, PRESSURE, MASS AND VORTICITY DATA FOR ALL PARTICLES                      
         ...
         do i=1,np
             read(iinp,*,end=300) xp(i),zp(i),yp(i),srf(i),alp(i)
            npp=npp+1
         enddo
300    np=npp
