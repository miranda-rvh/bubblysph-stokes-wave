...
c       initial data for SPH models                                                                     72
      ...
      real p(np_max),rho(np_max),beta(np_max),vort(np_max)
      real time(i_PART_counter_max), DT(i_PART_counter_max)
      integer nsurf(np_max),fltype(np_max)
      integer np_all(i_PART_counter_max)
      integer iinp,ioutp,dummy_int
      real dummy_real
...
c     %LOAD AND READ TIME,DT FROM FILE DT. THE FIRST HEADERLINE IN 
     ...
     &              i_PART_counter_max
          ...
          read(70,*,END = 76)dummy_real,dummy_int,
     & dummy_int,dummy_real,dummy_int,dummy_int,np_all(i_PART_counter)
...
c       % READ IN THE PART FILE FOR EACH FRAME
        ...
        if(ngrab.lt.10) then
           ...
           name_orig='../data_directory/BUBBLE'//supp1
        end if
        if(ngrab.ge.10.and.ngrab.lt.100) then
           write(supp2,'(i2)') ngrab
           name_orig='../data_directory/BUBBLE'//supp2
        end if
        if(ngrab.ge.100.and.ngrab.lt.1000) then
           write(supp3,'(i3)') ngrab
           name_orig='../data_directory/BUBBLE'//supp3
        end if
        if(ngrab.ge.1000.and.ngrab.lt.10000) then
           write(supp,'(i4)') ngrab
           name_orig='../data_directory/BUBBLE'//supp
        end if
        write(supp,'(i4.4)') ngrab

        name_vtu ='../paraview_files/BUBBLE'//supp//'.vtu'
...
c     % OUTPUT TO FILE IN VTU FORMAT 
     ...
     &ion= '//DQ//'0.1'//DQ//'  byte_order= '//DQ//'BigEndian'//DQ//'>'
        ...
        string1 = '   <PointData Scalars='//DQ//'radius'//DQ//
     &' Vectors='//DQ//'Velocity'//DQ//'>'
        write(ioutp,202)string1

c       % WRITE IN radius DATA             
        string2 = '    <DataArray type='//DQ//'Float32'//DQ//' Name='//D
     &Q//'radius'//DQ//' format='//DQ//'ascii'//DQ//'>'
        write(ioutp,202)string2
        do ii=1,np
          write(ioutp,*)p(ii)
        enddo
