...
c       initial data for SPH models                                                                     72
      ...
      real vp(np_max),a0(np_max),dummy
...
c     %LOAD AND READ TIME,DT FROM FILE DT. THE FIRST HEADERLINE IN 
     ...
     &              i_PART_counter_max
          ...
          read(70,*,END = 76)time(i_PART_counter),dummy
     & ,IT(i_PART_counter),DT(i_PART_counter),np_all(i_PART_counter)
...
c       % READ IN THE PART FILE FOR EACH FRAME
        ...
        if(ngrab.lt.10) then
           ...
           name_orig='../data_directory/SLICE'//supp1
        end if
        if(ngrab.ge.10.and.ngrab.lt.100) then
           write(supp2,'(i2)') ngrab
           name_orig='../data_directory/SLICE'//supp2
        end if
        if(ngrab.ge.100.and.ngrab.lt.1000) then
           write(supp3,'(i3)') ngrab
           name_orig='../data_directory/SLICE'//supp3
        end if
        if(ngrab.ge.1000.and.ngrab.lt.10000) then
           write(supp,'(i4)') ngrab
           name_orig='../data_directory/SLICE'//supp
        end if
        write(supp,'(i4.4)') ngrab

        name_vtu ='../paraview_files/SLICE'//supp//'.vtu'
...
c       % READ POSITION, VELOCITY, DENSITY, PRESSURE, MASS AND VORTICITY DATA FOR ALL PARTICLES                      
     ...
     &,p(i),vort(i),alpha(i),surf(i),a0(i)
            npp=npp+1
         enddo
300    np=npp
