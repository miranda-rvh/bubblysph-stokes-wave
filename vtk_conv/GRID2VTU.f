...
c       initial data for SPH models                                                                     72
      ...
      real vp(np_max),aBubbles(np_max),dummy
...
c     %LOAD AND READ TIME,DT FROM FILE DT. THE FIRST HEADERLINE IN 
     ...
     &                       dummy_int,np_all(i_PART_counter)
...
c       % READ IN THE PART FILE FOR EACH FRAME
        ...
        if(ngrab.lt.10) then
           ...
           name_orig='../data_directory/WGRID'//supp1
        end if
        if(ngrab.ge.10.and.ngrab.lt.100) then
           write(supp2,'(i2)') ngrab
           name_orig='../data_directory/WGRID'//supp2
        end if
        if(ngrab.ge.100.and.ngrab.lt.1000) then
           write(supp3,'(i3)') ngrab
           name_orig='../data_directory/WGRID'//supp3
        end if
        if(ngrab.ge.1000.and.ngrab.lt.10000) then
           write(supp,'(i4)') ngrab
           name_orig='../data_directory/WGRID'//supp
        end if
        write(supp,'(i4.4)') ngrab

        name_vtu ='../paraview_files/WGRID'//supp//'.vtu'
